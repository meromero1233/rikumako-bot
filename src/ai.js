...
export async function generateShortsResearch(digest) {
  const msg = await client.messages.create({
    model: 'claude-haiku-4-5-20251001',
    max_tokens: 1500,
    messages: [
...
以下はWeb検索で集めた、YouTubeショートの伸ばし方・人気動画に関する最新情報です。

【検索ダイジェスト】
${digest}

高島として、朝7時の「ショート研究レポート」をまとめてください。構成は以下：

■ 今日の戦略メモ
- 構成のポイント（2〜3個）
- アルゴリズムの要点（2〜3個）
- 脚本で心がけること（2〜3個）
※検索情報に基づいて具体的に。一般論だけで埋めないこと。

■ 参考にすべき動画・事例
都市伝説系／雑学系／トレンドニュース系、それぞれ最大3つ。
検索ダイジェストに実際に出てきたもの【のみ】挙げること。URLはダイジェスト内にあるものだけを使い、絶対に創作しない。
再生数の情報が記事にあれば添える。有力な事例が拾えなかったジャンルは「本日は有力な事例なし」と正直に書く。
各項目：タイトルor概要＋（あればURL）＋参考にすべき理由を1行。

■ 高島の本音（ここが一番大事）
以下を高島らしく、率直に、わかりやすく伝えること：
・AI生成の文章をそのまま使った脚本では、チャンネルのアイデンティティが消える
・AIに頼り切った動画は、そんなに甘く伸びない
・「作業」になった瞬間に伸びは止まる。こだわりを持って構成と脚本を作り込むことが何より大事
・AIはあくまで材料出しと壁打ち相手。最後に魂を入れるのはリクムとマコ、2人自身

全体は読みやすく、長すぎないこと。
冒頭にタイトルや日付の見出しは書かないこと（投稿側で付けるため）。いきなり「■ 今日の戦略メモ」から始めること。`,
      },
    ],
  });
